...
void MainWindow::showParts(MyMesh* _mesh, int indice, int r, int v, int b)
{
    // parcours des sommets
    VertexHandle start = _mesh->vertex_handle(indice);
    visited[indice] = true;

    HalfedgeHandle init_hh = _mesh->halfedge_handle(start);

    HalfedgeHandle hh = init_hh;
    FaceHandle fh;
    VertexHandle vh;
    while(true){
        //EdgeHandle eh = _mesh->edge_handle(hh);
        //_mesh->set_color(eh, MyMesh::Color(r, v, b));

        fh = _mesh->face_handle(hh);
        if(fh.is_valid()) _mesh->set_color(fh, MyMesh::Color(r, v, b));

        vh = _mesh->to_vertex_handle(hh);
        if(!visited.at(vh.idx())) showParts(&mesh, vh.idx(), r, v, b);
...
void MainWindow::on_pushButton_clicked()
{
    ...
    visited = std::vector<bool>(mesh.n_vertices(), false);
